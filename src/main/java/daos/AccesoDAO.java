...
public class AccesoDAO implements IAccesoDAO {
	...
	@Override
	public void añadirAcceso(String idCentro, String renfe, String metro, String coche) throws DAOException {
	...
	public Acceso recuperarAccesoPorIdCentro(int idCentro) throws DAOException {
		...
		Acceso accesos = new Acceso();
			...
			try {
				st = con.prepareStatement(DbQuery.getRecuperarAccesosPorIdCentro(idCentro));
				st.setInt(1, idCentro);
				rs=st.executeQuery();
				
				if (rs.next()){
	
					accesos = new Acceso(rs.getString(1), rs.getString(2), rs.getString(3)); 
