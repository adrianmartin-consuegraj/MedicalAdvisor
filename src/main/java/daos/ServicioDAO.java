...
public class ServicioDAO implements IServicioDAO {
	...
	@Override
	public void añadirServicio(String idCentro, String aparcamiento, String television, String restaurantes)
			throws DAOException {
	...
	public Servicio recuperarServicioPorIdCentro(int idCentro) throws DAOException {
		...
		Servicio servicios = new Servicio();
			...
			try {
				st = con.prepareStatement(DbQuery.getRecuperarServicioPorIdCentro(idCentro));
				st.setInt(1, idCentro);
				rs=st.executeQuery();
				
				while (rs.next()){
	
					servicios = new Servicio(rs.getString(1), rs.getString(2), rs.getString(3)); 
